...
import { Globe, Download, Info, Check, Moon, Sun, Monitor, Snowflake, Sparkles, Trash2, Database } from 'lucide-react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { cleanupAutoGeneratedExpenses, getDatabaseStats, resetAllData } from '../database/cleanup';
...
export const SettingsScreen = () => {
  ...
  const handleShowStats = () => {
    ...
    Alert.alert(
      '📊 Database Stats',
      `Total Expenses: ${stats.totalExpenses}\n` + `Auto-generated: ${stats.autoGeneratedExpenses}\n` + `Budgets: ${stats.budgets}\n` + `Recurring: ${stats.recurringExpenses}`,
      [{ text: 'OK' }]
    );
  };

  const handleCleanupAutoExpenses = () => {
    Alert.alert('🧹 Cleanup Auto-Generated', 'Delete all expenses created by the old recurring system?', [
  ...
  const handleResetAllData = () => {
    Alert.alert('⚠️ Reset All Data', 'This will delete ALL budgets and expenses. Recurring expenses will be kept.', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Reset',
        style: 'destructive',
    ...
    <View style={tw.style('flex-1', `bg-[${isDark ? colors.dark.bg : colors.light.bg}]`)}>
      <SafeAreaView edges={['top']} style={tw`flex-1`}>
        <ScrollView style={tw`flex-1`} contentContainerStyle={tw`pb-6`}>
          ...
          {/* 🐛 DEBUG Section - À supprimer en prod */}
          <View style={tw`px-6 mb-8`}>
            ...
            <View style={tw.style('rounded-2xl overflow-hidden', isDark ? `bg-[${colors.dark.card}] border border-[${colors.dark.border}]` : 'bg-white shadow-sm')}>
              ...
              <TouchableOpacity onPress={handleResetAllData} style={tw`flex-row items-center px-4 py-4`}>
                <View style={tw.style('w-11 h-11 rounded-xl items-center justify-center mr-4', `bg-red-500/20`)}>
                  <Trash2 size={20} color="#EF4444" strokeWidth={2.5} />
                </View>
                <Text style={tw.style('flex-1 text-base font-medium', 'text-red-500')}>Reset All Data</Text>
              </TouchableOpacity>
            </View>
          </View>
