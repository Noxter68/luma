...
import { Globe, Download, Info, Check, Moon, Sun, Monitor, Snowflake, Sparkles, Trash2, Database, RefreshCw, Sprout, Gem, LogOut } from 'lucide-react-native';
...
export const SettingsScreen = () => {
  ...
  const { isDark, colors, themeMode, setThemeMode, palette, setPalette } = useTheme();
  ...
  const handlePaletteChange = async (paletteId: 'nordic' | 'mauve' | 'sage' | 'amber') => {
  ...
  const handleCleanupDuplicateIncomes = () => {
    Alert.alert('🧹 Clean Duplicate Incomes', 'Remove duplicate incomes (same month/source/amount)?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: () => {
          ...
          refresh();
          Alert.alert('✅ Done', `${deletedCount} duplicate incomes removed`);
        },
      },
    ]);
  };
  ...
  const handleCleanupAutoExpenses = () => {
    Alert.alert('🧹 Cleanup Auto-Generated Expenses', 'Delete all expenses created by the old recurring system?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Clean',
        style: 'destructive',
        onPress: () => {
          ...
          refresh();
          Alert.alert('✅ Done', `${deletedCount} auto-generated expenses deleted`);
        },
      },
    ]);
  };
  ...
  const handleResetAllData = () => {
    Alert.alert('⚠️ Reset Data', 'Delete ALL budgets, expenses and incomes?\n\nRecurring expenses will be kept.', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Reset',
        style: 'destructive',
  ...
  const handleResetEverything = () => {
    Alert.alert('☢️ NUCLEAR RESET', 'Delete EVERYTHING including recurring expenses?\n\nThis cannot be undone!', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'DELETE ALL',
        style: 'destructive',
          ...
          Alert.alert('Are you sure?', 'Really delete EVERYTHING?', [
            { text: 'Cancel', style: 'cancel' },
            {
              ...
              onPress: () => {
                resetEverything();
                refresh();
                Alert.alert('✅ Done', 'Everything has been deleted');
              },
            },
          ]);
        },
  ...
  const handleSignOut = async () => {
    Alert.alert(locale === 'fr' ? 'Déconnexion' : 'Sign Out', locale === 'fr' ? 'Voulez-vous vraiment vous déconnecter ?' : 'Are you sure you want to sign out?', [
      { text: locale === 'fr' ? 'Annuler' : 'Cancel', style: 'cancel' },
      {
        text: locale === 'fr' ? 'Déconnexion' : 'Sign Out',
        style: 'destructive',
        onPress: async () => {
          try {
            setSigningOut(true);
            ...
            Alert.alert(locale === 'fr' ? 'Erreur' : 'Error', locale === 'fr' ? 'Impossible de se déconnecter' : 'Failed to sign out');
          } finally {
            setSigningOut(false);
          }
        },
    ...
    <View style={tw.style('flex-1', `bg-[${isDark ? colors.dark.bg : colors.light.bg}]`)}>
      <SafeAreaView edges={['top']} style={tw`flex-1`}>
        <ScrollView style={tw`flex-1`} contentContainerStyle={tw`pb-6`}>
              ...
              {PALETTES.map((pal) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <IconComponent size={24} color={isSelected ? colors.primary : isDark ? colors.dark.textSecondary : colors.light.textSecondary} strokeWidth={2} />
                    </View>

                    {/* Info */}
                    <View style={tw`flex-1`}>
                      <View style={tw`flex-row items-center justify-between mb-2`}>
                        <Text style={tw.style('text-base font-semibold', isSelected ? `text-[${colors.primary}]` : `text-[${isDark ? colors.dark.textPrimary : colors.light.textPrimary}]`)}>
                          {pal.label}
                        </Text>
                        {isSelected && (
                      ...
                      <Text style={tw.style('text-xs', `text-[${isDark ? colors.dark.textSecondary : colors.light.textSecondary}]`)}>{pal.description}</Text>
                    </View>
                  </TouchableOpacity>
                );
              })}
          ...
          {/* 🐛 DEBUG Section */}
          <View style={tw`px-6 mb-8`}>
            <Text style={tw.style('text-sm font-semibold uppercase tracking-wider mb-4', `text-[${isDark ? colors.dark.textTertiary : colors.light.textTertiary}]`)}>🐛 Debug Tools</Text>
            ...
            <View style={tw.style('rounded-2xl overflow-hidden', isDark ? `bg-[${colors.dark.card}] border border-[${colors.dark.border}]` : 'bg-white shadow-sm')}>
              ...
              <TouchableOpacity
                onPress={() => {
                  Alert.alert('🧹 Clean Recurring Incomes', 'Remove duplicate recurring incomes?', [
                    { text: 'Cancel', style: 'cancel' },
                    {
                      text: 'Clean',
                      style: 'destructive',
                      onPress: () => {
                        ...
                        refresh();
                        Alert.alert('✅ Done', `${deletedCount} duplicate recurring incomes removed`);
                      },
                    },
                  ]);
                }}
              ...
              <TouchableOpacity onPress={handleResetEverything} style={tw`flex-row items-center px-4 py-4`}>
                ...
                <View style={tw`flex-1`}>
                  <Text style={tw.style('text-base font-bold', 'text-red-900')}>☢️ NUCLEAR RESET</Text>
